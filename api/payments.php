...
try {
    ...
    // Update application to show online payment verified and advance status
    $update = $pdo->prepare("UPDATE applications SET status = 'Biometrics Received', processing_type = ?, amount = ?, payment_status = 'Paid', payment_method = ?, appointment_date = ?, appointment_location = ?, updated_at = CURRENT_TIMESTAMP WHERE application_id = ?");
    $update->execute([$processingType, $amount, $paymentMethod, $appointmentDate, $appointmentLocation, $applicationId]);
   
    echo json_encode([
        'success' => true,
        'applicationId' => $applicationId,
        'status' => 'Biometrics Received',
        'amount' => $amount,
